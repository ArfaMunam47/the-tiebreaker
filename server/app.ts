...
// Normalize Netlify function path prefix if routed directly
app.use((req: Request, _res: Response, next: NextFunction) => {
  if (req.url.startsWith("/.netlify/functions/api")) {
    const sub = req.url.replace("/.netlify/functions/api", "");
    req.url = sub.startsWith("/api") ? sub : (sub ? `/api${sub}` : "/api");
  }
  next();
});

// Middleware to sync database store before handling incoming requests
app.use(async (_req: Request, _res: Response, next: NextFunction) => {
  try {
    await syncDatabaseFromStorage();
  } catch (e) {
